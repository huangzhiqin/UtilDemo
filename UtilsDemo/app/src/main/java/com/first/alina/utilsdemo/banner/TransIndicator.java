...
public class TransIndicator extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    public TransIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTransType = ta.getInteger(R.styleable.TransIndicator_trans_indicator_types, TRANS_CIRCLE);
    ...
    public void addPagerData(BannerViewPagerBuilder builder, ViewPager viewPager) {
        if (builder != null) {
            mCount = builder.list.size();
            if (mCount == 0) {
                return;
            }

            mOpenView = builder.indicatorView;
            LayoutParams params = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT);
            params.setMargins(mLeftMargin, 0, 0, 0);
            GradientDrawable drawable = setDrawable(mDefaultcolor);
            removeAllViews();
    ...
    private GradientDrawable setDrawable(int color) {
